...
int ITG3200::read(float* temp, float * fx, float * fy, float * fz){
    ...
    x = (1-a)*x + a*(float)((int16_t)((buffer[0]<<8) + buffer[1]))/scaleFactor;
    y = (1-b)*y + b*(float)((int16_t)((buffer[2]<<8) + buffer[3]))/scaleFactor;
    z = (1-c)*z + c*(float)((int16_t)((buffer[4]<<8) + buffer[5]))/scaleFactor;
    *fx = x;
    *fy = y;
    *fz = z;
